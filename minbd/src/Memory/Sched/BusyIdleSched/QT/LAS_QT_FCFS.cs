...
{
    class LAS_QT_FCFS : LAS
    {

        public LAS_QT_FCFS(int total_size, Bank[] bank)
            : base(total_size, bank)
        {
            Console.WriteLine("LAS_QT_FCFS");
        }
        ...
        public override MemoryRequest get_next_req(MemCtlr mem)
        {
            ...
            for (int b = bank_index; b < bank_index + mem.bank_max; b++)  // for each bank
            {
                ...
                for (int j = 1; j < buf_load[b]; j++) {
                    if (!helper.is_MARK_RANK_FCFS(cur_req, buf[b, j], service_rank[cur_proc], service_rank[buf[b, j].request.requesterID]))
                        cur_req = buf[b, j];
                }

                if (next_req == null || !helper.is_MARK_RANK_FCFS(next_req, cur_req, service_rank[next_req.request.requesterID], service_rank[cur_req.request.requesterID])) {
                    next_req = cur_req;
                }
            }
            return next_req;
