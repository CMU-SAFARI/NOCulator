...
{
	public class Router_Bridge : Router
	{
		...
		public override bool productive(Flit f, int level)
		{
			...
			{
                // 1) Local ring: if its dest is not in the
                // current ring, send it onto the global ring.
                // 2) Global ring: if its dest is in the
                // current ring, drop it into the local ring.
				if (level == Local)
...
*/
			...
			{			
				...
				if (f != null && productive(f, Local) && Q_L2G[n].Count < bufferDepth_L2G && (local_preserve[n] == null || local_preserve[n] == LLinkIn[n].Out || Q_L2G[n].Count < bufferDepth_L2G - 1))
				{
					...
					{
	//			Simulator.stats.observerTriggered.Add(1);
						local_preserve[n] = null;
						local_observer[n] = null;
					}
					LLinkIn[n].Out = null;
