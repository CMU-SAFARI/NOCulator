...
{
    public class HR_8_8drop_Network : Network
    {
        public HR_8_8drop_Network(int dimX, int dimY) : base(dimX, dimY)
        {
            X = dimX;
            Y = dimY;
        }
      	public override void setup()
       	{
			if (Config.N != 64)
				throw new Exception("HR_8_8drop only suport 8x8 network");
            nodeRouters = new Router_Node[Config.N];
           	L1bridgeRouters = new Router_Bridge[32];
			L2bridgeRouters = new Router_Bridge[8];
			...
			{
				int next, pre;
				if (n == 0 | n == 6) {next = n * 4 + 6; pre = next - 1; }
				else if (n == 1 || n == 7) {next = n * 4 - 2; pre = next - 1;}
