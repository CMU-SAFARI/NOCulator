...
pktwidth = int(sys.argv[3])     # Width of the packet id field
...
link_bits = addrx_bits * 2 + addry_bits * 2 + pktwidth + seqwidth + 2
pkt_l = 0
pkt_h = pktwidth - 1
desty_l = pkt_h + 1
...
llimit = (maxaddrx + 1) + (maxaddry + 1) + 2**seqwidth
lbits = int(math.floor(math.log(llimit, 2)) + 1)
...
f.write("// [Valid][Gold][Seq][Src][Dest][Packet]\n")
...
f.write("`define gold_f\t" + str(gold) + "\n")
f.write("`define gold_on_mask\t" + str(link_bits) + "'d" + str(1 << (gold - 1)) + "\n")
f.write("`define gold_off_mask\t" + str(link_bits) + "'d" +
        str(((1<<link_bits)-1) & (~(1<<gold))) + "\n")
f.write("`define seq_f\t" + str(seq_h) + ":" + str(seq_l) + "\t// Bit position of sequence number\n")
f.write("`define seq_w\t[" + str(seq_h) + ":" + str(seq_l) + "]\t// Bit position of sequence number\n")
...
f.write("`define pkt_f\t" + str(pkt_h) + ":" + str(pkt_l) + "\n")
f.write("`define pkt_w\t[" + str(pkt_h) + ":" + str(pkt_l) + "]\n")
f.write("`define pkt_n\t" + str(pktwidth) + "\n")
f.write("\n")

f.write("// These are for gold packets and flits\n")
f.write("`define gold_w\t[" + str(seqwidth + addrx_bits + addry_bits - 1) + 
        ":0]\t// Bits for the gold register\n")
f.write("`define l_w\t\t[" + str(lbits-1) + ":0]\t// Bits for the l counter\n")
f.write("`define l_limit\t\t" + str(llimit) + "\t// Max value of l\n")
f.write("\n")
